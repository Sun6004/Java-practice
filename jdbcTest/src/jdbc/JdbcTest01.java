...
public class JdbcTest01 {
	 ...
	 *  ==> JDBC API버전 4 이상에서는 getConnevtion()메서드에서 자동으로 로딩해 주기 때문에 생략할 수 있다.
	 *  Class.forName("oracle.jdbc.driver.OracleDriver");
	 * 2. DB에 접속하기 ==> 접속이 완료되면 Connection객체가 반환된다.
	 *    DriverManger.getConnection() 메서드를 이용한다.
	 * 3. 질의 ==> SQL 문장을 DB서버로 보내서 처리를 하고 그 결과를 얻어온다.
	 ...
	 *   2) sql문이 select문이 아닐경우 (insert,delete,update문 등)에는 정수값을 반환한다.
	 *   	(반환되는 정수값은 실행에 성공한 레코드수를 말한다.)
	 * 5. 사용한 자원을 반납한다. ==> close()메서드를 이용
	 */
	...
	public static void main(String[] args) {
		...
		try {
			...
			//4.결과 처리
			//한 레코드씩 화면에 출력하기
			System.out.println("  == sql문 처리결과 ==");
			
			//ResultSet객체에 저장된 데이터를 꺼내오려면 반복문과 next()메서드를 이용한다.
			
			//rs.next() ==> ResultSet객체의 데이터를 가르키는 포인터를 다음번째 레코드 위치로
			//				이동 시키고 그 곳에 데이터가 있으면 true, 없으면 false를 반환한다.
			while(rs.next()) {
				//ResultSet객체의 포인터가 가르키는 곳에 데이터를 가져오는 방법.
				//형식1) rs.get자료형이름(int,string..)("컬럼명 또는 alias명")
				//형식2) rs.get자료형이름(int,string..)(컬럼번호) ==> 컬럼번호는 1부터시작.
