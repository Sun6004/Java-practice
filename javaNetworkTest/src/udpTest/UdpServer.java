...
/*
 * UDP 방식: 비연결 지향, 신뢰성없음. 데이터가 순서대로 도착한다는 보장 없음 하지만 TCP방식보다 속도가 빠름
 * 
 * -DatagramSocket객체와 DatagramPacket객체를 이용해서 통신한다.
 * 1.DatagramSocket: 데이터의 송수신과 관련된 작업을 수행한다.(우채부)
...
//클라이언트가 보낸 메세지를 받고, 받은 메세지를 그대로 클라이언트에게 보내는 예제
	...
	public static void main(String[] args) {
		try {
			...
			while(true) {
				...
				System.out.println();
				System.out.println("상대방의 IP정보: "+addr);
				...
				// 송신용 패킷 객체 생성
				// 	==> 전송할 데이터가 저장된 byte형 배열, 전승할 데이터의 길이(배열길이),
				//		상대방의 주소정보(InetAddress객체), 포트번호 이 4가지를
				// 		생성자의 인수값으로 지정하여 생성한다.	
				outpacket = new DatagramPacket(sendMsg, sendMsg.length, addr, port);
