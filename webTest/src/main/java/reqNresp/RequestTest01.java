...
public class RequestTest01 extends HttpServlet {
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		//post 방식으로 전달되는 문자데이터의 인코딩 방식 설정
		request.setCharacterEncoding("utf-8");
		
		//전달되는 데이터 받기
		//getParameter("파라미터명") ==> 해당 파라미터에 설정된 값을 가져온다. (가져온 값의 자료형은 String)
		String userName = request.getParameter("username");	
		String job = request.getParameter("job");
		
		//request.getParameterValues("파라미터명") ==> 파라미터명이 같은 것이 여러개일 경우에 사용한다.
		//		==>가져오는 값의 자료형은 String[]이 된다.
		//		==> 지정한 파라미터명이 없으면 null을 반환한다.
		//getParameterValues()를 이용해서 hobby로 설정된 form의 체크박스 값들을 모두 읽어오기
