 ...
 *  	(Servlet버전 3.0 이상에서는 애노테이션(@WebServlet)으로 설정할 수 있다.
...
public class ServletTest03 extends HttpServlet {
	...
	public void init() throws ServletException {
		System.out.println("Servlet: " + this.getServletName() + "에서의 init()메서드");
	}
	...
	protected void service(HttpServletRequest request, HttpServletResponse response) throws ServletException,IOException{
		...
		//방법1 ==> HttpSevle(부모객체)의 service()메서드로 위임하기
		//super.service(request, response); //부모의 service호출
		
		//방법2 ==> 클라이언트의 전송 방식(get,post)을 구분해서 직접메서드 호출하기
		String method = request.getMethod(); //클라이언트의 전송 박식 구하기
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		System.out.println("doGet메서드 시작");
		response.setCharacterEncoding("utf-8");
		response.setContentType("text/html; charset=utf-8");
		PrintWriter out = response.getWriter();
		out.println("<html><head><meta charset='utf-8'</head>" + "<body><h1 style='color:red;'>doGet메서드 처리내용</h1></body></html>");
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		response.setContentType("text/html; charset=utf-8");
		PrintWriter out = response.getWriter();
		out.println("<html><head><meta charset='utf-8'</head>" + "<body><h1 style='color:blue;'>doPost메서드 처리내용</h1></body></html>");
	}
